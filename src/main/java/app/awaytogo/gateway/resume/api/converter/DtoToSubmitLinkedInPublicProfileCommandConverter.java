package app.awaytogo.gateway.resume.api.converter;

import app.awaytogo.gateway.resume.domain.command.impl.SubmitLinkedInPublicProfileCommand;
import app.awaytogo.gateway.resume.api.dto.SubmitLinkedInPublicProfileCommandDto;
import app.awaytogo.gateway.resume.api.exception.ApiException;
import org.apache.commons.lang3.StringUtils;
import org.springframework.core.convert.converter.Converter;
import org.springframework.stereotype.Component;

import java.util.regex.Matcher;
import java.util.regex.Pattern;

@Component
public class DtoToSubmitLinkedInPublicProfileCommandConverter
        implements Converter<SubmitLinkedInPublicProfileCommandDto, SubmitLinkedInPublicProfileCommand> {
    private static final Pattern PROFILE_ID_PATTERN = Pattern.compile("linkedin\\.com/in/([^/]+)/?");

    @Override
    public SubmitLinkedInPublicProfileCommand convert(SubmitLinkedInPublicProfileCommandDto source) {
        Matcher matcher = PROFILE_ID_PATTERN.matcher(source.url());
        if (matcher.find()) {
            String pathValue = matcher.group(1);
            if (StringUtils.isNotBlank(pathValue)) {
                return SubmitLinkedInPublicProfileCommand.builder()
                        .resumeId(pathValue)
                        .build();
            }
        }
        throw new ApiException("We couldn't extract a LinkedIn profile ID from the provided URL.");
    }
}
